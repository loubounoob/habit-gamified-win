            ...
            <>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground">{item.selectedOptions.map(o => o.value).filter(v => v !== 'Default Title').join(' • ')}</p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t">
                <Button onClick={handleCheckout} className="w-full" size="lg" disabled={items.length === 0 || isLoading || isSyncing}>
                  {isLoading || isSyncing ? <Loader2 className="w-4 h-4 animate-spin" /> : <><ExternalLink className="w-4 h-4 mr-2" />Commander</>}
                </Button>
              </div>
            </>
          )}
